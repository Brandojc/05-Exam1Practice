...
def problem2a(circle, rectangle, window):
    ...
    x= rectangle.get_lower_right_corner()
    x2= rectangle.get_upper_left_corner()
    window.render(.05)
    window.continue_on_mouse_click()
    line = rg.Line(x,x2)
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n - 1):
        left_upper = rg.Point(rect.get_upper_left_corner().x - delta, rect.get_upper_left_corner().y - delta)
        right_lower = rg. Point(rect.get_lower_right_corner().x + delta, rect.get_lower_right_corner().y + delta)
        rect = rg.Rectangle(left_upper,right_lower)
        rect.attach_to(win)
    win.render(.06)
